...
CAPTIONS_ZH = {
    ...
    "[FIGURE 5": "图5. 训练轨迹、稳定性与样本效率。（a）每种主要学习器包含5个训练种子；细线表示单个种子，粗线表示中位数，阴影带表示四分位距。（b）事后D6稳定性和D7样本效率得分。这些训练维度是确认性最终路线终点的补充，而不是重新定义该终点。",
...
SUPP_FIGURES_ZH = [
    ...
    ("图S6", FIGS_ZH / "supplementary" / "S06_zh.png", "7种论文可用学习变体的训练轨迹。曲线汇总每个模型5个种子、3,000个回合的训练过程。"),
    ("图S7", FIGS_ZH / "supplementary" / "S07_zh.png", "7个归一化维度及事后100分综合得分。该得分具有探索性并依赖权重，不是确认性终点。"),
    ("图S8", FIGS_ZH / "supplementary" / "S08_zh.png", "事后综合分析中第一名占比对训练权重和运行下限的联合敏感性。"),
...
def english_reference_lines() -> list[str]:
    lines = (DOCS / "EAAI_manuscript_source_v3.md").read_text(encoding="utf-8").splitlines()
...
def main() -> None:
    ...
    produced = [
        save_doc("EAAI_manuscript_source_v3.md", "EAAI_manuscript_anonymized_v3.docx", main=True),
        save_doc("EAAI_manuscript_source_v3.md", "EAAI_manuscript_two_column_reading_proof_v3.docx", "Author-formatted reading proof — not publisher typeset", main=True, reading_proof=True),
        save_doc("EAAI_title_page_source_v3.md", "EAAI_title_page_v3.docx"),
        save_doc("EAAI_supplementary_source_v3.md", "EAAI_supplementary_material_v3.docx", supplement=True),
        save_doc("EAAI_highlights_source_v3.md", "EAAI_highlights_v3.docx"),
        save_doc("EAAI_cover_letter_source_v3.md", "EAAI_cover_letter_v3.docx", compact=True),
        save_doc("EAAI_manuscript_zh_v3.md", "EAAI_manuscript_zh_v3.docx", main=True, chinese=True),
        save_doc("EAAI_title_page_zh_v3.md", "EAAI_title_page_zh_v3.docx", chinese=True),
        save_doc("EAAI_supplementary_zh_v3.md", "EAAI_supplementary_material_zh_v3.docx", supplement=True, chinese=True),
        save_doc("EAAI_highlights_zh_v3.md", "EAAI_highlights_zh_v3.docx", chinese=True),
        save_doc("EAAI_cover_letter_zh_v3.md", "EAAI_cover_letter_zh_v3.docx", chinese=True),
    ]
    report = {
        "preset": "EAAI author manuscript v3",
    ...
    (QA / "docx_build_profile_v3.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
