...
const rules = [
  ["H01","H","Official guide","Single-column Word author manuscript","Apply to EAAI_manuscript_anonymized_v4.docx","Applied"],
...
const applications = [
  ["Page size","A4","H / plan v4","English and Chinese single-column manuscripts"],
  ["Body text","Times New Roman 11 pt, 1.5 spacing","Plan v4","English submission manuscript"],
  ["Chinese body","宋体 + Times New Roman for Latin/numerals","Plan v4","Chinese author-review manuscript"],
  ["Headings","Black, left aligned, sentence case","S + plan v4","All v4 documents"],
...
title(sum,"EAAI 12-paper format comparison v4","H = official hard rule; S = stable recurring convention; O = publisher or paper-specific feature. Scientific content and sentences are never copied.","F");
...
sum.mergeCells("A12:F14"); sum.getRange("A12").values = [["The 12 papers are publisher-formatted two-column articles, but the EAAI author guide controls submission. Therefore the v4 single-column manuscript is the submission file; the two-column file is an explicitly labelled reading proof. Publisher branding and article metadata are excluded."]]; sum.getRange("A12:F14").format = {wrapText:true,verticalAlignment:"top",font:{size:10,color:ink}};
...
title(ap,"v3 application profile","Concrete format decisions derived from official rules, stable conventions and the user-approved v4 plan.","D"); ap.getRange("A4:D4").values=[["Element","v3 setting","Basis","Files"]]; ap.getRange("A5:D14").values=applications; header(ap.getRange("A4:D4")); body(ap.getRange("A5:D14")); ap.getRange("A:D").format.columnWidth=28; ap.getRange("B:B").format.columnWidth=48; ap.getRange("C:D").format.columnWidth=32;

await exportAndRender(formatWb,"EAAI_12_Format_Comparison_v4.xlsx",[["Summary","A1:F14"],["Paper Format","A1:N16"],["Rules","A1:F15"],["Application","A1:D14"]]);
...
await exportAndRender(termWb,"EN_ZH_Terminology_Glossary_v4.xlsx",[["Summary","A1:E10"],["Terms","A1:G34"]]);
